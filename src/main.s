...
; edi starts as zero so execution can fall through -> finally -> fetch
...
; 20
OP:       db        0xE9, finally-(target+2+5), 0, 0
...
; 16
LOAD:     db        0x1A, 0xE9, finally-(target+3+5), 0

          db        0x0F, 0xBE              ; movsx ebx,byte[edx] - 0F BE 1A
SYSTEM:   db        0x0F, 0xBF              ; movsx ebx,word[edx] - 0F BF 1A
...
; 10
STORE:    db        0x0A, 0xE9, finally.ret-(target+3+5), 0
...
; 20
BRANCH:   db        0xE9, finally.ret-(target+2+5), 0, 0
